...
export default function AdminNonUSVerifications() {
  ...
  const fetchUserVerifications = async (userId) => {
    setLoading(true);
    try {
      let url = `${API_BASE}/api/admin/verifications/user/${userId}`;
      if (filter !== 'ALL') {
        url += `?status=${filter}`;
      }
  ...
  const fetchVerifications = async () => {
    ...
    try {
      ...
      if (filter === 'PENDING_REVIEW') {
        url = `${API_BASE}/api/admin/verifications/pending`;
      } else if (filter === 'APPROVED') {
        url = `${API_BASE}/api/admin/verifications/approved`;
      } else if (filter === 'REJECTED') {
        url = `${API_BASE}/api/admin/verifications/rejected`;
      } else {
        url = `${API_BASE}/api/admin/verifications/pending`;
      }
  ...
  window.open(`${API_BASE}/api/admin/verifications/${selectedVerification.id}/download`, '_blank');
};
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/admin/verifications/${selectedVerification.id}/approve?adminUsername=admin`, 
        {
          method: 'POST',
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/admin/verifications/${selectedVerification.id}/reject?adminUsername=admin&notes=${encodeURIComponent(rejectReason)}`, 
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Non-US Citizen Verifications</h2>
    ...
    {verifications.map(verification => {
      ...
      return (
        <tr key={verification.id}>
          ...
          <td style={styles.td}>{verification.documentType}</td>
          <td style={styles.td}>{verification.documentFileName}</td>
          <td style={styles.td}>{formatDate(verification.submittedAt)}</td>
          <td style={styles.td}>
      ...
      {showDetailsModal && selectedVerification && (
        <div style={styles.modalOverlay} onClick={() => setShowDetailsModal(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <h3 style={styles.modalTitle}>Non-US Citizen Verification #{selectedVerification.id}</h3>
            ...
            <div style={styles.section}>
              <h4 style={styles.sectionTitle}>Document Information</h4>
              <div style={styles.detailRow}>
                <span style={styles.detailLabel}>Type:</span>
                <span style={styles.detailValue}>{selectedVerification.documentType}</span>
              </div>
              <div style={styles.detailRow}>
                <span style={styles.detailLabel}>File Name:</span>
                <span style={styles.detailValue}>{selectedVerification.documentFileName}</span>
              </div>
              <div style={styles.detailRow}>
                <span style={styles.detailLabel}>Document Number:</span>
                <span style={styles.detailValue}>{selectedVerification.documentNumber || 'N/A'}</span>
              </div>
              <div style={styles.detailRow}>
                <span style={styles.detailLabel}>Issuing Country:</span>
                <span style={styles.detailValue}>{selectedVerification.issuingCountry || 'N/A'}</span>
              </div>
              <div style={styles.detailRow}>
                <span style={styles.detailLabel}>Expiry Date:</span>
                <span style={styles.detailValue}>{selectedVerification.expiryDate || 'N/A'}</span>
              </div>
            </div>
